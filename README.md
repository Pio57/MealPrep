...
1. **Lander** (`Lander`): schermata di apertura, con asset ed emoji al centro personalizzati con un'animazione di rotazione a orbita (parte lasciata libera dal brief).
2. **Budget Selection** (`BudgetSelection`): slider per il budget settimanale, range **25–150 €**. Lo slider nativo non permette di controllare lo spessore della traccia, quindi sopra a uno slider reso trasparente (solo per thumb/touch) è disegnata una traccia personalizzata (due `View` assolute: sfondo più riempimento proporzionale al valore).
3. **Dietary Needs** (`DietaryNeeds`): esigenze dietetiche tra Nessuna, Vegetariano, Vegano, Pescetariano, Senza glutine e Senza lattosio. Ogni opzione è mappata su un filtro reale del catalogo (`src/Core/Catalog/dietaryNeeds.ts`), che sia una label positiva del prodotto (es. "Vegetarian"), un allergene da escludere (es. "Glutine", "Latte") o un reparto da escludere (es. Pescetariano esclude i reparti carne/salumi ma include il pesce).
4. **Nutritional Goals** (`NutritionalGoals`): obiettivi nutrizionali tra Ricco di proteine e Povero di zuccheri/grassi/carboidrati/sale. Anche qui ogni opzione è una soglia reale sui valori nutrizionali per 100g del prodotto (`src/Core/Catalog/nutritionalGoals.ts`).
5. **Weekly Meal Plan** (`MealPlan`): piano di 7 giorni generato via LLM (OpenAI, workflow in `src/Core/Llm/mealPlanWorkflow.ts`). Il catalogo viene prima filtrato lato client secondo le esigenze e gli obiettivi selezionati, poi ridotto a un sottoinsieme rappresentativo (max 25 prodotti per reparto, 220 totali) per stare in un budget di token ragionevole. Il prompt impone un piano interamente in italiano, l'uso esclusivo di `productId` reali dal sottoinsieme, un `totalPrice` entro il budget scelto e **3 pasti a giorno** (colazione, pranzo e cena, oltre il minimo di 1/giorno richiesto dal brief) con convenzioni italiane, quindi niente pasta a colazione. Ogni pasto mostra nome, tipo pasto, tempo di preparazione, porzioni, prezzo, ingredienti e ricetta passo-passo, e i pasti dello stesso giorno si scorrono con uno swipe orizzontale sulla card, mentre le linguette in alto cambiano giorno con selezione automatica del giorno corrente all'apertura.
